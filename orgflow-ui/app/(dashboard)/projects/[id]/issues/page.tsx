...
export default function ProjectIssuesPage({ params }: Props) {
  ...
  const loadIssues = useCallback(async () => {
    return listProjectQualityIssues(
      projectId,
      issuesFilterStateToListQuery(filters, effectiveRole)
    );
  }, [projectId, filters, effectiveRole]);
  ...
  const {
    ...
    error,
    reload,
  } = useOrgQuery(`projects/${projectId}/issues?${filterKey}`, loadIssues, {
    ...
    return (
      <main className="of-dashboard-page">
        <div className="mx-auto max-w-6xl">
          <PageHeader
            title="ליקויים"
            description="אין הרשאה לצפייה ברשימת הליקויים בפרויקט זה"
          />
        </div>
      </main>
    );
  ...
  const projectLabel = project?.project_name?.trim() || "הפרויקט";
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <PageHeader
          ...
          description={
            contractorView
              ? contractorIssuesPageDescription()
              : `מעקב ליקויים חיים בפרויקט ${projectLabel} - ממצאים מדוחות שטח שנסגרו`
          }
          actions={
            <button
              type="button"
